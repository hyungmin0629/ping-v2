...
**후보 풀 4명 미만** — 스코프를 한 단계 낮추고(CLASS→SCHOOL→GLOBAL), GLOBAL 에서도
모자라면 그 질문을 내지 않는다. 실제로 쓴 스코프는 `vote_item.candidate_scope` 에 남는다.
(2026-07-29 결정, [[DECISIONS]])
...
| 8 | **W7 배포** | 외부인이 링크로 투표 완료 | **다음** |
| 9~13 | P3~P7 파이프라인 | NEIS → BigQuery → 대시보드 | |
| 14 | W8 학교 정보 | 내 학교·반의 급식·시간표·공지가 보인다 | P3 필요 |

**예상 기간** — W0~W7까지 주 10~15시간 기준 **3~5주**. 가장 오래 걸리는 단계는 W5(투표)다.

**다음 작업**: 순서 8 — W7 배포.
Vercel 배포, 개인정보처리방침(수집 항목: 닉네임·성별·소속 / 만 14세 미만 금지),
지인 초대. 배포 전에 `db/seed_test_friends.py --clean` 으로 더미를 지우고
`db/rls/verify.py` 를 통과시킨다. 초대 링크(`/add/[코드]`)도 이때 다시 만든다.
