...
그 결함을 닫은 스키마를 새로 설계했고(MVP 기준 40 테이블, FK 76), 합성 데이터로
정합성을 검증했다. **여기까지가 완료된 상태다.**

<span class="dim">합성 데이터는 판본을 거듭했다 — 이 문장을 쓸 때는 786만 행(v1)이었고,
2026-08-05 현재 **1억 2,370만 행(v5)** 이다. 판본별 경위는 [[reports|리포트 목록]].</span>
...
**2026-08-18 현재 P7 만 남았다.** P0~P2·P4·P5·P6 이 완료됐고, P3(NEIS)는 수집은
되지만 DAG 화가 남았다. 웹앱은 W19 까지 끝나 실유저 데이터가 실제로 흐르고 있어,
합성과 실유저가 **같은 파이프라인**을 탄다(`_source` 로 갈린다).
...
| ~~지목한 사람에게 익명 메시지~~ | 있음 | **W15 에서 열었다.** 30자·1회성으로 좁히고 신고를 함께 붙였다. 차단 화면은 여전히 없다 |
...
| 검증 | `db/rls/verify.py` 의 게시판 29종 (W9 당시 전체 115항목 · 지금 201) |
...
├── bigquery/              # stg 뷰 10 / mart 표 8 변환 SQL + build.py
├── qa/checks/
└── data/synthetic/
```
...
| `rejected_friend_recommendations.reason` | `MUTUAL_CONTACT` 미사용 | 연락처를 안 받으므로 |
...
`verify.py`(RLS 201항목)와 `verify_load.py`(행 수 대조)가 대표적이다.
...
| — | P1 합성 데이터 | 786만 행, 정합성 17종 통과. **지금은 지움**(2026-07-31) | **완료** |
...
| 10 | **P4 BigQuery 적재** | 40테이블 · 원천별 행 수 대조 통과. 합성을 지워 지금은 실유저만 | **완료** |
| 11 | **P5 품질검증** | 자동 생성 검사 430항목 · 두 원천 다 위반 0 | **완료** |
| 12 | **P6 stg/mart** | stg 뷰 10개 · mart 표 8개 · 주간 필터와 전주 대비 검증 | **완료** |
| 13 | P7 대시보드 | 루커에 `mart` 를 붙여 5쪽을 그린다 | 다음 차례 |

> **P6 의 첫 작업은 대리키였다.** 두 원천의 id 가 실제로 겹친다
> (`app_user` 16개, `vote_item` 26개). raw 를 `JOIN ... USING(id)` 로 조인하면
> 실유저의 투표에 합성 유저의 프로필이 붙고, **오류는 나지 않는다.**
> stg 가 `_source || '-' || id` 형태의 `user_key` 를 만들어 이 실수를 구조적으로 막는다.
>
> 그레인은 **대시보드의 기간 필터가 정했다** — 주 단위로 좁혀 전주와 비교하려면
> 월 그레인·미리 계산한 비율·미리 센 distinct 를 쓸 수 없다.
> [[mart-grain-for-weekly-filter]] · 절차는 [[ops-p5-p7]].
...
| 20 | **W14 선택형 힌트** | 5+1 선택형 · 상세 창 · 광고 무료 열기 | **완료** |
| 21 | **W15 1회성 답장** | 20하트 · 30자 · 한 번 · 신고 포함 | **완료** |
...
그 플래그는 `app_user` 에만 있어서 나머지 39개 테이블에 쓸 수 없고, 두 원천의
