...
**다음 작업**: 순서 7 — W6 받은 투표.
알림 목록, 힌트 구매(하트 차감, 누진 200→300→500→1000), 답변 공개.
`vote_received` 는 직접 조회가 막혀 있고 `my_vote_received` 뷰로만 나오므로,
힌트를 살 때 `reveal_status` 를 올리는 RPC가 필요하다.
