...
## 마트에 무엇을 구웠나 — 2026-08-18 에 정해졌다

⚠️ **아래 제안은 전부 실제로 반영됐다.** stg 뷰 10개와 mart 표 8개가 서 있고,
갭 3건도 거기서 닫혔다 — 시간대는 `stg` 가 KST 날짜를 미리 싣고, 제재는
`valid_until` **시각**으로 자르고, 광고 힌트는 `is_paid_hint`/`is_ad_hint` 로 갈렸다.
그레인을 그렇게 정한 이유는 [[mart-grain-for-weekly-filter]], 층의 절차는 [[ops-p5-p7]].

**새 분석은 `raw` 가 아니라 `stg` 에서 시작한다.** 아래 규칙이 뷰 안에 들어 있다.

원래의 제안은 다음과 같았다 — 무엇을 왜 구웠는지 읽는 데 도움이 되므로 남긴다.
