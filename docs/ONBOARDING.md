...
전체 규모(2만 명·12개월·학교 50개)는 **옵션 없이** `--config synthetic-v2.yaml`
만 주면 됩니다 — 규모가 설정 파일에 들어 있습니다. **생성 14분**(2026-08-06 실측)
이고 CSV 11GB 가 나옵니다. 적재는 그 뒤 별도입니다. 처음에는 작게 돌려 보세요.
...
**시드가 같으면 같은 데이터가 나옵니다.** 시드는 설정 파일의 `meta.seed`
(지금 `20260803`)이고 `--seed` 로 덮어쓸 수 있습니다. 2026-08-06 에 실제로
확인했습니다 — 같은 명령을 두 번 돌려 **CSV 33개가 해시까지 전부 일치**했습니다.

재현이 되므로 **덤프를 주고받을 필요가 없습니다.** 팀원은 저장소만 있으면
같은 데이터를 각자 만들 수 있고, "내 쪽에서만 이상하다"도 가릴 수 있습니다.
분포 값은 구 서비스 실측치에서 가져온 것이라, 바꾸면 정합성 검사가 걸릴 수 있습니다.

```bash
python generator/load.py --truncate
```

위의 작은 시험(500명·1개월)이면 **90초쯤** 걸립니다. 끝나면 시퀀스 재동기화와
워터마크 되돌리기가 자동으로 이어서 돕니다 — **이 둘을 건너뛰면 오류 없이
조용히 틀립니다**(가입 시 PK 충돌 · 12개월치가 하루로 뭉침). 그래서 CSV 를
손으로 `\copy` 하면 안 됩니다.

**확인** — `적재 완료 — 총 N 행` 이 나오면 성공입니다. N 은 만든 규모에 따라
다릅니다. 작은 시험은 100만 행대, **전체 규모는 1억 2,369만 행**입니다.
...
python db/run_sql.py db/rls/profile.sql
python db/run_sql.py db/rls/withdraw.sql
python db/run_sql.py db/rls/topup.sql
python db/run_sql.py db/rls/hangul.sql
python db/run_sql.py db/rls/hints.sql
...
**201항목이 전부 통과해야 합니다.** 하나라도 실패하면 남의 데이터가 보인다는
뜻이니 다음으로 넘어가지 마세요.
...
| `raw/` | **원본.** 회의록·구 서비스 분석. 고치지 않습니다 |
| `docs/decisions/` | **왜 그렇게 했는지** 56개. 하나가 한 파일 |
...
| 층 | 어디 | 누가 고치나 |
|---|---|---|
| **원본** | `raw/` — 회의록·구 서비스 분석 | **사람만.** 회의에서 정한 게 바뀌어도 회의록은 그대로 둡니다 |
| **위키** | `docs/` — 결정·표·운영 | 에이전트가 만듭니다 |
| **규약** | `CLAUDE.md` | 그 일을 어떻게 하는지 |

**무엇을 찾을 때 어디로 가나:**

| 알고 싶은 것 | 어디 |
|---|---|
| 왜 이렇게 했지? | `DECISIONS.md` 색인 → 해당 결정 파일 |
| 이 표는 뭐지? | `docs/tables/<표이름>.md` — **한 장에 다 있습니다** |
| 이 작업 어떻게 하지? | `docs/ops/` |
| 문서가 다 어디 있나 | `docs/index.md` |

**옵시디언으로 열면** 저장소 루트가 볼트입니다. `Ctrl+O` 로 노드 이름을 치는 게
제일 빠르고, 그래프 뷰에서 결정·표·원본이 색으로 나뉩니다.

⚠️ **손으로 고치면 안 되는 파일이 있습니다** — `docs/index.md`, `docs/tables/`,
`docs/erd.md` 는 스크립트가 만듭니다. 고쳐도 다음 생성 때 날아갑니다.
고칠 것이 있으면 원본(DDL 주석 등)을 고칩니다.
