...
테스트 중에 남겨둬도 된다. 후보는 내 친구 중에서만 뽑히고 친구를 맺으려면 초대
   코드를 알아야 하므로, 더미를 붙이지 않은 사람은 더미를 마주칠 수 없다.
   영향은 더미를 붙인 계정의 후보 풀이 더미로 채워진다는 것뿐이다.
   그 계정으로 일반 이용자처럼 써보고 싶을 때 --clean 하면 된다.
