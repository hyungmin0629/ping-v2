...
**P0·P1·P2·W0 완료** (2026-07-29) → 다음은 **W1 Supabase 구축 + RLS**

| 완료 | 결과 |
|---|---|
| P0 스키마·DDL | 제약 위반 16종 차단 검증 |
| P1 합성 데이터 | 786만 행 생성 (25초) |
| P2 적재 | Postgres 적재 (89초) |
| W0 익명 인증 대응 | **42 테이블** / 정합성 17종 통과 / 개인정보 컬럼 0개 |

W0에서 적용된 것: `app_user`가 `nickname` + `invite_code` 구조가 됐고,
개인정보 컬럼(전화·이메일·비번·실명)이 하나도 없음을 쿼리로 확인했다.
`user_contact`는 `70_deferred_v2.sql`로 분리해 MVP에서 생성하지 않는다.
