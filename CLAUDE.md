...
합성 데이터를 만들기 전에 팀이 정할 것은 [[SYNTHETIC-PLAN]].
...
## ⚠️ 합성 데이터는 지금 없다 (2026-07-31)

**전부 지웠다.** 로컬 `pgtest` 컨테이너, BigQuery 의 786만 행, `data/synthetic/`
CSV 411MB 까지.
...
**만드는 코드는 그대로다.** `generator/` 와 설정도 남아 있고, 스키마와 어긋난
곳이 없는 것을 확인했다. 명령 두 줄이면 다시 만들어진다(생성 25초 + 적재 89초).

**다시 만드는 것은 팀과 스키마를 확정한 뒤다.** [[TEAM-PLAN]] 의 순서가
*스키마 점검 → ERD 기반 데이터 생성*이고 지금이 그 점검이 막 끝난 지점이다.
여기서 혼자 만들어봤자 팀이 스키마를 또 건드리면 두 번 일이 된다.
...
- 합성 데이터 규모: **유저 5,000명 / 3개월치** (설정값. 지금 실체는 없다)
