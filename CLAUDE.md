...
"언제 왜 마음이 바뀌었나"가 남는다. 자세한 것은 [[README|raw/README]].
...
4. `python db/wiki_index.py` 로 색인을 다시 만든다
5. 원본의 frontmatter 를 `ingested: true` 로 바꾼다
6. [[log|docs/log.md]] 에 한 줄 남긴다

### query — 물어보면 위키에서 찾아 답한다

전문을 훑지 않는다. **`docs/decisions/` 를 grep 해 파일명만 받고, 이름을 보고
골라 그 노드만 읽는다.** 색인(1,810토큰)까지 읽을 필요도 보통은 없다.
...
- 설계 결정이 내려지면 **`docs/decisions/<슬러그>.md` 를 새로 만들고**
  [[DECISIONS]] 색인에 한 줄 넣는다. 기존 결정을 뒤집는 것이면 옛 파일에
  `status: superseded` 와 `superseded_by` 를 단다 — 뒤집힌 결정이 그대로
  살아 있는 것이 제일 위험하다.
