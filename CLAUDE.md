...
[[row-cap-to-query-cap]] 에 있다. 판본별 실측은 `docs/EDA-sample-1m-v{1,2,3}.pdf`.

### 최종 생성 완료 (2026-08-04)

20,000명 · 12개월 · 50학교로 **1억 4,126만 행 · CSV 7.4GB** 를 10분 16초에
만들었다. 3단계(500 → 2,000 → 최종) 중 2단계를 건너뛰고 최종으로 갔다.

**3국면 성장 곡선을 여기서 붙였다.** 설정의 `growth` 블록은 있었지만 **읽는
쪽이 없어** 첫 국면의 가입 비중만 쓰이고 나머지 넷은 버려지고 있었다.
가입 비중과 활동 배수 두 갈래로 걸었다 — [[growth-curve-two-channels]].

| 국면 | 목표 가입 비중 | 실측 |
|---|---|---|
| launch_spike (9월) | 35% | 35.1% |
| decline (10~12월) | 15% | 15.3% |
| trough (1월) | 3% | 3.0% |
| recovery (2~4월) | 22% | 21.5% |
| growth (5~7월) | 25% | 24.9% |

⚠️ **1개월 샘플은 곡선이 꺼진다**(국면과 기간이 안 겹친다). 그래서 v1~v3 실측이
그대로 유효하고, 회귀 확인도 **바이트 단위로 동일**했다.

⚠️ **아직 미구현** — 세션 중도 이탈 비율(완료율 98.6%, 목표 78%),
`schools.adoption`(순차 확산), `retention.reactivation_rate`(휴면 복귀 코호트).
뒤의 둘은 설정에 값만 있고 읽는 쪽이 없다 — 성장 곡선과 **같은 종류의 함정**이다.

⚠️ **저점 대비 회복 MAU 는 2.18배**로 목표(2.5배)에 조금 못 미친다.
2,000명 시험에서는 2.51배였다.
