...
**2026-08-05 에 v4 를 확정했다** — 20,000명 · 12개월 · **1억 8,922만 행**.
정합성 17종 · 이상치 9종 위반 0, 목표 지표 10개 전부 달성
([검수 리포트](EDA-final-12m-v4.pdf) · 23쪽).

⚠️ **시각은 UTC 로 저장된다.** 시간대를 집계할 때 KST 로 바꾸지 않으면
봉우리가 **9시간 밀린다**(밤 22시가 낮 13시로 보인다).

### ⚠️ 이 데이터로 **할 수 없는** 것이 15가지 있다

확정 직전 정밀 EDA 에서 **결함 1건 · 경계 5건**이 나왔다. 고치지 않고
**[[confirm-v4-with-known-limits|문서화하기로 정했다]]** —
고치려면 전면 재생성이라 Q28 2차 증분 배치가 또 밀린다.

⚠️ **결함 — 하루 투표 세션 수에 상한이 없다.** 하루 최다 **2,243세션**,
투표에 쓴 시간 최대 **406.8시간/일**, 24시간 초과가 **1,251 유저-일(67명)**.
정합성 17종·이상치 9종이 **이걸 안 묻는다.**
영향은 꼬리 0.5%에 갇혀 있으므로 **두 규칙만 지키면 나머지는 살아 있다** —
**①상위 1% 세그먼트를 보지 않는다**(그 167명이 사람이 아니다)
**②활동 집중 지표의 절대값을 인용하지 않는다**(상위 10% = 89.5% · 지니 0.936).
**v5 의 1순위다.**

| 경계 (5건) | 무엇이 불가능해지나 |
|---|---|
| 접속 세션과 투표 세션이 **서로 독립** | **"앱 열기 → 투표" 퍼널 불가.** 겹침 3.84%. 접속 세션은 볼륨·시간대 지표로만 쓴다 |
| 시간 간격이 **전부 균등분포** | 체류시간·응답속도·열람지연의 **분포·분위수 인용 불가.** 평균만 유효 |
| 탈퇴·BAN 이 활동을 **안 멈춘다** | 탈퇴자 **88.7%** 가 탈퇴 후에도 하트를 쓴다. **코호트에서 시각으로 잘라야 한다** |
| 7개 축에 **설계된 차이가 없다** | 요일 · 질문 · 후보 슬롯 · 앱버전 · 게시판 카테고리 — 검정하면 **반드시 기각된다** |
| `reveal_status='REVEALED'` **0건** | 이름까지 공개한 12,103건이 `PARTIAL` 로 남는다. `hint_type='FULL_NAME'` 으로 조인해 우회 |

⚠️ **휴면·복귀는 반드시 `vote_session` 으로 잰다.** `user_session` 으로 재면
최대 공백이 11일 20시간이라 **복귀자가 0명으로 나온다.**

**분석 전에 리포트 13장("할 수 있는 것 / 없는 것")을 먼저 본다.**
할 수 있다 17종 · 주의해야 한다 7종 · **할 수 없다 14종**으로 갈라 놨다.
