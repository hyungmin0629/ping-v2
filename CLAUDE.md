...
**P0·P1·P2·W0~W5 완료** (2026-07-29) → 다음은 **W6 받은 투표**
...
⚠️ **클로즈드 테스트 전에 반드시 `--clean` 한다.** 남겨두면 실유저가 존재하지 않는
사람에게 투표하고, 그 사람은 받은 투표를 영영 열어볼 수 없다.
더미는 `is_synthetic = true` 로 찍히므로 BigQuery 에서도 걸러낼 수 있다.
...
- ~~후보 풀이 4명 미만일 때의 처리~~ → **W5에서 해소.** 스코프를 한 단계 낮추고,
  GLOBAL 에서도 모자라면 그 질문을 내지 않는다. 경위는 [[DECISIONS]].
