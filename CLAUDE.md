...
**P0·P1·P2·W0~W4 완료** (2026-07-29) → 다음은 **W5 투표**
...
- **초대 링크 미구현.** W7 배포 때 다시 만든다. 그때 `/add/[코드]` 라우트가 개발
  서버에서 렌더링 워커째 죽던 문제부터 재현·해결할 것. 경위는 [[DECISIONS]].

- **W5 시험에는 더미 친구가 필요하다.** 투표 화면이 열리려면 친구가 5명이어야 하는데
  창을 다섯 개 띄울 수는 없다. `auth_user_id` 는 nullable 이므로 로그인 계정 없이
  프로필 행만 만들면 된다. `is_synthetic = true` 로 찍고, 클로즈드 테스트 전에 지운다
  — 안 지우면 실유저가 존재하지 않는 사람에게 투표한다.
