...
**P0~P7·W0~W19 완료**(2026-08-25). 남은 것은 **P3 의 DAG 화 하나뿐이다.**

stg 뷰 10개 · mart 표 8개 위에 **루커 5쪽**이 붙었다 — KPI·시계열 / 유저·코호트 /
퍼널·분포 / 하트 / 신고·적체. **설계대로 붙었다**: 주간 규칙 넷으로 그레인을 잡고,
여덟 표가 같은 차원 이름을 쓰고, `sido_iso` 를 stg 에 미리 실어 둔 덕이다.
절차와 주간 규칙 넷은 [[ops-p5-p7]], 그레인의 이유는 [[mart-grain-for-weekly-filter]].
전체 기록은 [완주 리포트](PROJECT-CLOSING.pdf).

⚠️ **루커에는 `mart` 만 연결한다.** `raw` 를 붙이면 화면을 새로 그릴 때마다
1억 2천만 행을 읽어 무료 한도 1TiB/월이 사라진다.

앱은 배포돼 있고 실데이터가 BigQuery 까지 흐른다. 초대를 미룰 이유가 없어졌다
— 증분이 처음부터 흐르므로 소급 적재가 필요 없다.

**주간 보고서가 매주 월요일 저절로 나간다**(2026-08-25). 적재부터 Drive 배송까지
GitHub Actions 에서 돈다 — 아래 「주간 보고서」 절.
...
## ⚠️ DDL 만 읽으면 안 된다 — 주석이 낡아 있다

`apply.py` 가 **DDL + 마이그레이션을 전부** 돌리므로 `db/ddl/*.sql` 은 현재
스키마가 아니라 **출발 상태**다. 진실은 파일 하나가 아니라 **순서**에 있다.

2026-08-13 에 실제로 걸렸다 — `30_question_vote.sql` 의 주석을 근거로 힌트
요금을 **200→300→500→1000 누진**이라고 말했다. W14(마이그레이션 006)가 반년 전에
없앤 요금제다. 현행은 **기본 5종 각 20하트 + 이름 공개 100하트**이고, 값은
`db/rls/hints.sql` 의 `hint_cost()` 하나가 정한다.

**표를 인용하기 전에 그 표를 건드린 마이그레이션이 있는지 본다.**

```
grep -l "ALTER TABLE <표>" db/migrations/*.sql
```

낡은 블록에는 경고를 그 자리에 남겼다(30·60·10). **주석만 고친다 —
SQL 문장은 재생성의 출발점이라 현행으로 "고치면" 마이그레이션이 깨진다.**
경위와 `doc_lint` 로 못 잡는 이유는 [[ddl-comments-rot-with-migrations]].
...
⚠️ **매칭된 노드를 전부 읽으면 쪼개기 전보다 나쁘다.** 2026-08-25 재측정
(`db/token_bench.py`, 노드 91개) — 골라 읽기 **1,268** · 한 파일 시절 grep
**4,484** · 매칭 노드 전부 읽기 **31,655** 토큰이다. 전부 읽기가 한 파일 시절의
**7.1배**다(56개 시절엔 3.5배였다). 노드가 늘수록 규율을 어겼을 때의 벌금이 커진다.
"합성" 같은 낱말은 이제 **91개 중 47개**에 걸린다.
**구조가 아니라 규율이 이득을 만든다.**
...
측정 기준값 — `python db/token_bench.py` 로 언제든 다시 잰다.
두 시점을 **같은 토크나이저로** 잰 값이다(읽기 창 ±25줄):

| | 골라 읽기(D) | 한 파일 grep(B) | 매칭 전부(E) | D 절감 |
|---|---|---|---|---|
| 노드 56개 (2026-07-31) | 972 | 2,535 | 8,822 | 61.6% |
| 노드 91개 (2026-08-25) | **1,268** | **4,484** | **31,655** | **71.7%** |

**위키가 커질수록 골라 읽기의 이득도 커진다** — 위키 자체는 2.6배가 됐는데
D 는 30%만 늘었다. 되돌릴 이유는 아직 없다.
⚠️ 옛 기록(1,263 · 1,928 · 6,907)은 토크나이저가 기록되지 않아 이 값과 직접
비교하지 않는다. 위 표는 두 시점을 다시 잰 것이다.
⚠️ **다음에 볼 것은 D 안의 파일 목록 비중이다** — 13% → 24% 로 올랐다.
흔한 낱말이 위키 절반에 걸리기 시작하면 목록만으로 노드 본문만큼 든다.
...
- 스키마 구조는 **DDL이 진실**이다. 문서에 테이블 정의를 복사하지 않는다.
  ⚠️ 단 **`db/ddl/` 폴더 하나가 아니라 DDL + 마이그레이션이 진실**이다 —
  아래 「DDL 만 읽으면 안 된다」 참조.
