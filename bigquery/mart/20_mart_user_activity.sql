...
--
-- 크기를 재고 골랐다 — 유저×일 상한은 654만 행이다. 굽기 전 **접속(세션) 기준**
-- 으로 세니 262,540행이었고(세션이 병합돼 유저당 접속일이 평균 14일),
-- ⚠️ **실제로 구워진 표는 2,288,001행 · 384 MiB 다** — 아래 '활동일'의 정의가
--    접속일보다 넓기 때문이다. 상한의 35% 라 일 그레인을 유지한다.
--    **재고 결정한 대상과 실제로 담는 대상이 같은지 확인할 것.**
--
